...
class SizeClass
{
	...
	{
		size_t alignum = 1 << align;
		return ((size + alignum - 1) >> align) - 1;
	}
	inline static size_t _Roundup(size_t size, size_t align)
	{
		size_t alignum = 1 << align;
		return (size + alignum - 1)&~(alignum - 1);
	}
public:
	//控制在12%左右的内碎片浪费
	...
	//[8*1024, 64*1024] 1024字节对齐 freelist[128, 184)
